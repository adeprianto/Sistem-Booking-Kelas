<div class="container-fluid">
    ...
    foreach ($data_booking as $booking) { ?>
        <?php if (($booking->waktu_mulai <= date("H:i")) && ($booking->waktu_akhir >= date("H:i"))) { ?>
            <h2 class="text-center text-light"><span class="badge badge-danger font-weight-bold">Kelas Sedang Digunakan Dari Pukul : <?= $booking->waktu_mulai; ?> Sampai <?= $booking->waktu_akhir; ?></span></h2>
            <?php $used = true;
            break;
        } ?>
    <?php } ?>
    ...
    <?php if (!$used) { ?>
        <h2 class="text-center text-light"><span class="badge badge-success font-weight-bold">Kelas Sedang Tidak Digunakan Saat Ini</span></h2>
    <?php } ?>
